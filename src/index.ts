...
/**
 * Interface for the response from the xAI image generation API
 */
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    const response = await xaiApi.post<XAIImageResponse>("/images/generations", {
      model: "grok-2-image",
      prompt,
      n,
    ...
    if (axios.isAxiosError(error)) {
      ...
      throw new McpError(
        ...
        `xAI API Error (${statusCode}): ${errorMessage}`
      );
    }
